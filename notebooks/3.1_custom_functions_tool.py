...
from eba_regulatory_agent.mcp import ToolInfo
...
def get_databricks_token() -> str:
    """Get a Databricks auth token without requiring token creation permissions."""
    try:
        dbutils_obj = globals()["dbutils"]
        ctx = dbutils_obj.notebook.entry_point.getDbutils().notebook().getContext()
        return ctx.apiToken().get()
    except Exception:
        return w.tokens.create(lifetime_seconds=1200).token_value
...
class SimpleAgent:
    ...
    def __init__(self, llm_endpoint: str, system_prompt: str, tools: list[ToolInfo]):
        ...
        self._client = OpenAI(
            api_key=get_databricks_token(), base_url=f"{w.config.host}/serving-endpoints"
        )
